...
class Register extends CI_Controller {
 ...
 function validation()
 {
  ...
  if($this->form_validation->run())
  {
   ...
   $id = $this->RegisterM->insert($data);
   if($id > 0)
   {
    $subject = "Please verify email for login";
    $message = "
    <p>Hi ".$this->input->post('user_name')."</p>
    <p>This is email verification mail from A&J Boutique Login Register system. For complete registration process and login into system. First you want to verify you email by click this <a href='".base_url()."register/verify_email/".$verification_key."'>link</a>.</p>
 ...
 function verify_email()
 {
  if($this->uri->segment(3))
  {
   ...
   if($this->RegisterM->verify_email($verification_key))
   {
